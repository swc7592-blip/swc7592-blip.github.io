#!/usr/bin/env python3
"""
X Economy Bot - Twitter API v2 (Final Fixed Version)
...
def get_headers():
    ...
    return {
        ...
        "User-Agent": "v2EconomyBot/1.0-Final-Fixed"
    }
...
def get_user_tweets(username, user_id):
    """Get recent tweets from a user (Twitter API v2 - Corrected)"""
    headers = get_headers()
    if not headers:
        return []

    # CORRECT ENDPOINT: /2/users/{user_id}/tweets (NOT /timelines/reverse)
    ...
    try:
        ...
        if e.code == 429:
            ...
            return []
        if e.code == 401:
            log(f"Authentication Error for {username}: Invalid or expired token", level="ERROR")
            return []
...
def generate_economy_report(economy_tweets):
    """Generate economy report (Simplified - Fixed Syntax)"""
    if not economy_tweets:
        return "경제 관련 트윗이 없습니다."
    
    timestamp = datetime.now().strftime("%Y년 %m월 %d일 %H:%M (KST)")
    
    # Simple Sort: Assign to variable first, then sort
    # This avoids complex lambda parsing
...
"""
    ...
    if any(kw in keyword_counts for kw in ["Market", "Stocks", "Economy"]):
        report += "• **주식/시장:** 경제 지표 및 시장 동향 관련 토론 활성\n"
...
def save_report(report):
    ...
    filename = DATA_DIR / f"economy_report_fixed_{timestamp}.txt"
...
def main():
    """Main execution flow"""
    log("=" * 50)
    log("🐦 X Economy Bot - START (Fixed Version)")
    log("=" * 50)
